    ...
    if (releaseEntry && cwd && process.env.DEVLOG_RELEASE_GUARD !== "0") {
      try {
        ...
        if (items.length > 0) {
          ...
          out.push("");
          out.push(`🛑 ${items.length} مهمة مفتوحة — لا يجوز إصدار release بوجود أي مهمة مفتوحة:`);
          ...
          out.push("");
          out.push("الإصلاح: أَغلق كل #N بـ -(done) / -(dropped) / -(bug fix) / -(security fix) في الرد التالي،");
          out.push("ثم أعد إصدار -(release). أو تجاوز مؤقتاً بـ DEVLOG_RELEASE_GUARD=0.");
          out.push("");
          out.push("✗ الـrelease tag لم يُسجَّل.");
          out.push("══════════════════════════════════════");
      ...
      if (!r.ok) await enqueueTags(body);
        ...
        try {
          ...
          if (resp.releaseBlocked) {
            ...
            out.push("", "أَغلِق كل عنصر بـ -(done)/-(dropped)/-(bug fix)/-(security fix) (بالرقم، أو بالنص للعناصر بلا #N)،",
          ...
          if (resp.rollback) {
            ...
            const manifest = rb.restoredTo
              ? `استُرجِع المانيفست إلى ${rb.restoredTo}`
              : "لم يُسترجَع المانيفست (لا مرجع سابق) — تحقّق يدوياً إن لزم";
            process.stderr.write(
              `\n[devlog rollback]\n↩ أُزيل الإصدار ${rb.version}: ${manifest}` +
              `${rb.htmlDeleted ? "، حُذِفت الصفحة" : ""}${rb.indexRebuilt ? "، أُعيد بناء الفهرس" : ""}.\n`);
          ...
          if (Array.isArray(resp.closed) && resp.closed.length) {
            const lines = resp.closed.map(c => `✓ أُغلق #${c.num} — ${c.text}`);
            process.stderr.write(`\n[devlog closure]\n${lines.join("\n")}\n`);
          ...
          if (resp.verifyHint && Array.isArray(resp.verifyHint.closers) && resp.verifyHint.closers.length
            ...
            process.stderr.write(
              `\n[devlog verify]\n💡 أغلقتَ (${verbs}) بلا تشغيل أي اختبار في هذه الجلسة. ` +
              `«التحقّق» = دليل مُلاحَظ (اختبار ناجح في المحادثة)، لا قراءة الكود. شغّل الاختبار للتأكيد.\n`);
          ...
          if (resp.release) {
            ...
            const bumps = (rel.bumped || []).map(u => `${u.file} ${u.from}→${u.to}`).join("، ") || "لا مانيفست لرفعه";
            const downgrades = (rel.rejected || []).map(u => `${u.file} ${u.current}→${u.attempted}`).join("، ");
            const out = [
              "════════ DevLog Release ════════",
              `✓ الإصدار ${rel.version} سُجِّل في DevLog.`,
              `رفع النسخة: ${bumps}`,
              ...(downgrades ? [`⚠ رُفض تنزيل النسخة (المانيفست أحدث): ${downgrades}`] : []),
              `HTML/changelog: ${rel.htmlGenerated ? "أُنشئ ✓" : "لم يُنشأ"}`,
              "",
  ...
  try {
    ...
    if (shouldEnforceStandards({
      ...
      const out = [
        ...
        "",
        `التصنيفات المنطبقة غير المُغطّاة: ${need}`,
      ...
      if (items.some(it => MANIFEST.test(it.file_path || ""))) {
        ...
        if (violations.length && !served.includes(sig)) {
          ...
          const out = [
            ...
            "(تعطيل: DEVLOG_STANDARDS_CHECK=0)",
            "═════════════════════════════════════════",
          ].join("\n");
          process.stderr.write(`\n${out}\n`);
