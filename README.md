...
- [Bun](https://bun.sh/) 1.1+ (server runtime)
...
The "فحص أمني" / Scan button checks each dependency against its ecosystem's official registry (npm, crates.io, PyPI, Go, Packagist) and flags outdated packages — showing the latest version, its release date, and a ⏳ warning for releases newer than 7 days. It's built in: no external service, no API key, no extra process to run.

> Scope: this reports **outdated versions**, not CVEs. Native registry data carries version info only — it can't report known vulnerabilities. (Earlier builds had an optional external Vuln API for CVE data; dropped in v2.5.0 to keep setup to a single process.)
...
All data stays local. The server only listens on `127.0.0.1`. No telemetry, no external calls. `.devlog-data/` and `.devlog/` are excluded from git by the bundled `.gitignore` — keep them that way; they contain your full activity history.
