...
- `/api/failure-class-backfill` — failure-class backfill (#998). GET `?project=|?cwd=[&limit=30&offset=0]`: the closed bug/security reports whose closer carries no class, oldest closure first, each with its material (report text, prose around report and fix, the closer’s `cause`, fix files) plus coverage counts (`total`, `byCloser`, `backfilled`, `more`). POST `{assignments:[{closerId, class}], confirm?}`: without `confirm:true` a validated preview (`rows` from→to), nothing written; the whole batch is refused (422, `refused[]` with reasons) on an unknown id, a non-report closer, a word outside `src/failure-class.ts`, or a class the closer wrote itself — a previously backfilled class may be replaced. Confirmed writes archive every touched row to the `undone` stream FIRST (503 and nothing written if the archive fails), then set `failureClass` + `failureClassBackfilled:true` (audited). Driver: `bun scripts/backfill-failure-class.ts list|apply`
