...
| **Stop release-guard** | `-(release)` in your response | ANY open item exists (todo, bug, security, plan step). Refuses to persist the release. Close everything first, or `DEVLOG_RELEASE_GUARD=0` to override. |
...
| `-(release) vX.Y.Z — summary` | Release. **Only when the user explicitly asks.** |
...
## Releases & GitHub — not your job

Don't run `git push` / `git commit` / any GitHub CLI, and don't issue `-(release)` tags unless the user explicitly asks. A separate session handles git/GitHub. Don't hand-edit `version` in `package.json` / `Cargo.toml`: when `-(release) vX.Y.Z` is emitted, the server bumps the manifest, writes the release HTML, and updates the changelog.
