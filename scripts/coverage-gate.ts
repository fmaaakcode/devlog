...
// coverage reads ~0 and would be a meaningless gate. The exclusion is ENFORCED
// by `isGated` below (#1162 / audit round 10 F-9.162): the header claimed it
// while the filter excluded nothing — server.ts was absent from the report only
// because no test happened to import it, and the first in-process import would
// have dragged the aggregate under the floor with no code regression at all.
//
// The decision is a pure function (`evaluate`) so the gate itself has a test
// (test/wave9-gates.test.ts); the CLI wrapper at the bottom only reads the file
// and exits.

import { readFileSync } from "node:fs";

export const OVERALL_FLOOR = 80; // src-only aggregate (currently ~82.6%)

// basename → minimum line-coverage %. Keep sorted by risk.
export const SENSITIVE: Record<string, number> = {
...
/** Files the aggregate deliberately leaves out — exercised only through a
 *  subprocess, so their in-process line count is noise, not signal. */
export const EXCLUDED = ["src/server.ts"] as const;
...
export type Rec = { file: string; lf: number; lh: number };

export function parseLcov(raw: string): Rec[] {
  const records: Rec[] = [];
  let cur: Partial<Rec> = {};
...
/** Under src/ and not on the exclusion list. */
export function isGated(file: string): boolean {
  const f = file.replace(/\\/g, "/");
  if (!/(^|\/)src\//.test(f)) return false;
  return !EXCLUDED.some(x => f === x || f.endsWith(`/${x}`));
}

export interface GateResult { ok: boolean; overall: number; rows: string[]; failures: string[] }

export function evaluate(records: Rec[], floors = SENSITIVE, overallFloor = OVERALL_FLOOR): GateResult {
  const srcRecords = records.filter(r => isGated(r.file));
  ...
  const overallOk = overall >= overallFloor;
  if (!overallOk) failures.push(`overall src/ ${overall.toFixed(2)}% < ${overallFloor}%`);
  rows.push(`${overallOk ? "PASS" : "FAIL"}  overall src/            ${overall.toFixed(2).padStart(6)}%  (floor ${overallFloor}%)`);
  ...
  return { ok: failures.length === 0, overall, rows, failures };
}
...
if (import.meta.main) {
  ...
  const result = evaluate(parseLcov(raw));
