...
const TYPE_LABEL: Record<DocType, [en: string, ar: string]> = {
  report: ["Report", "تقرير"],
...
function fmtDate(iso: string): string {
  try {
    return new Date(iso).toLocaleString(currentLang() === "ar" ? "ar" : "en", { year: "numeric", month: "long", day: "numeric" });
  } catch { return iso; }
}

function docCss(): string {
  // Logical (inline-start/end) properties so one sheet serves both directions
  // (#895) — the page dir comes from renderDocHtml's <html> attributes.
  return `
  ${DL_THEME_ROOT}
  * { box-sizing: border-box; }
  ...
  .dl-h1 { font-size:1.5em; padding-inline-start:10px; border-inline-start:3px solid var(--c-update); }
  .dl-h2 { font-size:1.25em; padding-inline-start:10px; border-inline-start:3px solid var(--c-decision); }
  .dl-h3 { font-size:1.08em; color:var(--text2); }
  .dl-h4,.dl-h5,.dl-h6 { font-size:0.95em; color:var(--text2); }

  .dl-list { padding-inline-start:22px; }
  .dl-list li { margin-bottom:5px; }
  .dl-task-list { list-style:none; padding-inline-start:8px; }
  .dl-task { display:flex; align-items:flex-start; gap:8px; padding:4px 8px; border-inline-start:2px solid var(--border); margin-bottom:4px; border-radius:4px; }
  .dl-task[data-checked="true"] { border-inline-start-color:var(--c-built); }
  ...
  .dl-code { background:var(--bg3); border:1px solid var(--border); border-radius:8px; padding:14px 16px; overflow-x:auto; font-size:0.85em; line-height:1.55; direction:ltr; text-align:left; }
  ...
  .dl-table th,.dl-table td { border:1px solid var(--border); padding:8px 12px; text-align:start; }
  .dl-table thead th { background:var(--bg3); color:var(--text); font-weight:600; }
  .dl-table tbody tr:nth-child(odd) td { background:var(--bg3); }

  .dl-callout { border-inline-start:3px solid var(--text2); background:var(--bg3); padding:10px 14px; border-radius:6px; }
  .dl-callout p { margin:0; }
  .dl-callout-note      { border-inline-start-color:var(--c-decision); }
  .dl-callout-info      { border-inline-start-color:var(--c-decision); }
  .dl-callout-tip       { border-inline-start-color:var(--c-built); }
  .dl-callout-warning   { border-inline-start-color:var(--c-security); }
  .dl-callout-important { border-inline-start-color:var(--c-update); }
...
export function renderDocHtml(meta: DocMeta, markdownBody: string): string {
  ...
  const typeLabel = L(...TYPE_LABEL[meta.type]);
...
<main id="dl-root" class="dl-doc dl-doc-${meta.type}" data-spec-version="${SPEC_VERSION}" data-doc-type="${meta.type}">
  ...
  <header class="dl-doc-header">
    ...
    <div class="dl-doc-meta">
      <span><b>${L("Project", "المشروع")}:</b> ${escapeHtml(meta.project)}</span>
      &middot;
      <time datetime="${escapeHtml(meta.createdAt.split("T")[0])}">${escapeHtml(fmtDate(meta.createdAt))}</time>
      ${meta.updatedAt !== meta.createdAt ? `&middot; <span>${L("updated", "محدّث")} ${escapeHtml(fmtDate(meta.updatedAt))}</span>` : ""}
    </div>
  </header>
...
${body}
  ...
  <footer>
    ${L("Generated by", "تم التوليد بواسطة")} <code>DevLog</code> · <code>doc:${meta.type}</code>
  </footer>
