...
لاسترجاع التاريخ المسجَّل («لماذا اخترنا X؟»، «هل مررنا بهذا قبل؟») أصدر \`-(ask:search) <سؤال>\` — يجيبك DevLog بأفضل التاقات المطابقة (قرارات، insights، بلاغات مغلقة مع إصلاحاتها). \`-(ask:search) all: <سؤال>\` يوسّع البحث لكل المشاريع المتتبَّعة. فضّله على إعادة اشتقاق قرار ماضٍ أو إعادة التحقيق في مشكلة محلولة. وللسؤال بالزمن بدل الموضوع («وش صار آخر شي؟») أصدر \`-(ask:recent)\` — خلاصة الجلسة السابقة (تاقاتها وملفاتها وأوامرها الفاشلة؛ الجلسة السائلة مستثناة). \`-(ask:recent) 3\` = آخر 3 جلسات، و\`-(ask:recent) 7d\` = آخر 7 أيام.
