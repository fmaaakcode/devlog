...
export function formatClosureMessage(r: ClosureCheckResult, lang: Lang = currentLang()): string {
  if (!r.unclosed.length && !r.warnings.length) return "";
  const ar = lang === "ar";
  const lines: string[] = [];
  const label = (tag: string) => tag === "plan-step" ? (ar ? "خطوة" : "plan-step") : tag;
  if (r.unclosed.length) {
    lines.push(ar
      ? `✗ ${r.unclosed.length} عمل بدون إقفال (تطابق قوي):`
      : `✗ ${r.unclosed.length} work item(s) done but not closed (strong match):`);
    for (const m of r.unclosed) {
      lines.push(`  • -(${label(m.item.tag)}) #${m.item.num}: ${m.item.content.slice(0, 80)}`);
      lines.push(ar
        ? `    يطابق: -(built) ${m.built.slice(0, 80)}`
        : `    matches: -(built) ${m.built.slice(0, 80)}`);
      lines.push(ar
        ? `    أضف: -(${closureTagFor(m.item.tag)}) #${m.item.num}`
        : `    add: -(${closureTagFor(m.item.tag)}) #${m.item.num}`);
    }
  }
  if (r.warnings.length) {
    lines.push(ar
      ? `⚠ ${r.warnings.length} عمل قد يحتاج إقفال (تطابق ضعيف — تجاهل لو غير ذي صلة):`
      : `⚠ ${r.warnings.length} work item(s) that may need closing (weak match — ignore if unrelated):`);
