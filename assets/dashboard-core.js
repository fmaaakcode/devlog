...
//
// Dispatch table, not an if/else ladder (audit C7) — the same shape
// that won server-side in parse-tags' ASK_ROWS/RESPONSE_ROWS: adding
// an action is a row here, not another branch in a 90-line chain.
// Every handler gets (el, e); unknown actions fall through silently,
// exactly like the old chain's missing else.
const ACTIONS = {
    "select-project": (el) => selectProject(el.dataset.project),
    "delete-project": (el, e) => { e.stopPropagation(); deleteProject(el.dataset.project); },
    "rename-project": (el, e) => { e.stopPropagation(); renameProject(el.dataset.project); },
    "open-sessions": (el) => openSessionsPanel(el.dataset.project),
    "cleanup-tombstones": () => cleanupTombstones(),
    // Portable project bundle: ⤓ downloads (Content-Disposition does the
    // rest), ⤒ picks a bundle file and merge-imports it.
    "export-project": (el, e) => { e.stopPropagation(); window.location.assign(`${API}/api/project-export/${encodeURIComponent(el.dataset.project)}`); },
    "import-project": () => importProjectBundle(),
    "kill-pid": (el) => killPid(parseInt(el.dataset.pid, 10), el.dataset.project),
    "refresh-processes": (el) => refreshProcesses(el.dataset.project),
    "toggle-plan": (el) => {
    ...
    "set-todos-tab": (el) => { setTodosTab(el.dataset.key); renderTodosCard(false); },
    "set-plans-tab": (el) => { setPlansTab(el.dataset.key); renderActivePlanCard(activeProject, false); },
    "toggle-plan-upcoming": (el, e) => {
        e.stopPropagation();
    ...
    "expand-history": () => { expandHistory(activeProject); refreshActiveView(true); },
    "close-sessions-modal": () => { const m = document.getElementById('sessionsModal'); if (m) m.remove(); },
    "clear-injection-override": () => clearInjectionOverride(),
    "toggle-injection": (el) => toggleInjection(el.dataset.key, el.dataset.value === 'true'),
    "show-injection-content": (el) => showInjectionContent(el.dataset.id),
    "switch-inj-scope": (el) => switchInjScope(el.dataset.scope),
    "show-vulns": (el) => showVulnsModal(el.dataset.project, el.dataset.lib),
    "show-vulns-tag": (el) => showVulnsFromTag(el.dataset.project, el.dataset.content),
    "close-vulns-modal": () => { const m = document.getElementById('vulnsModal'); if (m) m.remove(); },
    "noop": () => { /* swallow clicks inside a modal so the backdrop doesn't close it */ },
    ...
    "rescan-project": (el) => rescanProject(activeProject, el),
    "vuln-scan": (el) => vulnScan(activeProject, el),
    "open-injection-panel": () => openInjectionPanel(activeProject),
    "open-standards-panel": () => openStandardsPanel(activeProject),
    "kill-server": (el) => killServer(el),
    "open-updates-popup": () => openUpdatesPopup(),
    // #448: the version chip's click-through to the full releases page
    // (the hover popover shows the latest summary inline).
    "open-releases": () => window.open(`${API}/releases/${encodeURIComponent(activeProject)}`, "_blank", "noopener"),
    // #490: live preview of the NEXT release — served under the same
    // base as the baked pages so its relative links resolve.
    "open-release-preview": () => window.open(`${API}/releases/${encodeURIComponent(activeProject)}/preview.html`, "_blank", "noopener"),
    // Client-facing status report — rendered live server-side; the
    // browser tab is the review/print/save surface.
    "open-client-report": () => window.open(`${API}/api/client-report?project=${encodeURIComponent(activeProject)}`, "_blank", "noopener"),
    // بطاقة أداء النماذج — نافذة عند الطلب من شريحة الرأس (فكرة 1، 2026-07-27).
    "open-model-stats": () => openModelStatsPanel(activeProject),
    "close-model-stats-modal": () => { const m = document.getElementById('modelStatsModal'); if (m) m.remove(); },
    ...
    "open-target-file": () => openTargetFile(),
    "ignore-target": () => ignoreTarget(),
    "close-injection-panel": () => closeInjectionPanel(),
    "close-standards-panel": () => closeStandardsPanel(),
};
...
export const langColors = {
    ...
    // Brand colors stay literal; the non-brand default is the dashboard
    // --blue token, resolved (not var()) because callers concat an
    // alpha suffix onto it (`${color}18`).
    Vue: "#42b883", Svelte: "#ff3e00", default: cssVar("--blue", "#118ab2")
};
...
async function checkDaemonFreshness() {
    try {
        ...
        bar.id = 'freshnessBar';
        bar.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:1000;display:flex;gap:12px;align-items:center;justify-content:center;padding:7px 16px;font-size:13px;background:#2a2210;color:var(--gold);border-bottom:1px solid #c98500';
        const msg = document.createElement('span');
        msg.textContent = tr("fresh.stale");
        const btn = document.createElement('button');
        btn.textContent = tr("fresh.restart");
        btn.style.cssText = 'background:#c98500;color:var(--bg);border:0;border-radius:6px;padding:4px 12px;font-size:12px;cursor:pointer;font-family:inherit';
        btn.onclick = async () => {
            btn.disabled = true;
