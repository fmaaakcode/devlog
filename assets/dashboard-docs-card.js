        ...
        const MEM_TYPES = new Set(['user', 'feedback', 'project', 'reference']);
        const typeLabel = (type) => MEM_TYPES.has(type) ? tr(`mem.${type}`) : type;
        ...
        const docTypeLabel = (name, type) => isStudyDoc(name) ? tr("doc.study")
            : DOC_TYPES.has(type) ? tr(`doc.${type}`) : String(type || '');
            ...
            try {
                ...
                p.docFiles = [...docs]
                    ...
                    .map(d => ({
                        ...
                        docType: d.type,
                        name: d.name,
                        description: `${docTypeLabel(d.name, d.type)}${d.updatedAt ? tr("doc.updatedAt", { date: String(d.updatedAt).slice(0, 10) }) : ''}`,
                        body: d.preview || tr("doc.noPreview"),
                    }));
        ...
        export function buildDocsHtml(project) {
            ...
            let h = `<div style="font-size:0.6em;color:var(--text2);margin-bottom:6px;text-transform:uppercase;letter-spacing:0.5px">${tr("card.docs")} <span style="opacity:0.6">${total}</span></div>`;
            h += '<div style="overflow-y:auto;flex:1;min-height:0">';

            if (mem.length > 0) {
                h += `<div style="font-size:0.7em;color:var(--text2);margin-bottom:4px">${tr("doc.memSection")}</div>`;
                mem.forEach((m, i) => {
                    ...
                    h += `<div class="mem-row" data-mem-kind="mem" data-mem-idx="${i}" style="display:flex;align-items:center;gap:6px;padding:3px 4px;font-size:0.7em">
                        ...
                        <span style="font-size:0.8em;color:${color};flex-shrink:0">${esc(typeLabel(m.type))}</span>
                    </div>`;
                });
            }

            if (docs.length > 0) {
                h += `<div style="font-size:0.7em;color:var(--text2);margin:${mem.length ? '8px' : '0'} 0 4px">${tr("doc.docSection")}</div>`;
                docs.forEach((d, i) => {
                    const color = isStudyDoc(d.name) ? 'var(--gold)' : 'var(--emerald)';
                    h += `<div class="mem-row" data-mem-kind="docs" data-mem-idx="${i}" data-doc-slug="${esc(d.slug || '')}" title="${tr("doc.openFull")}" style="display:flex;align-items:center;gap:6px;padding:3px 4px;font-size:0.7em">
                        <span style="width:6px;height:6px;border-radius:50%;background:${color};flex-shrink:0"></span>
            ...
            if (total === 0) {
                h += `<div style="color:var(--text2);font-size:0.7em">${tr("card.noFiles")}</div>`;
            }
