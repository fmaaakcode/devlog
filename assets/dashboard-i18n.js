...
import { FEATURES_DICT } from "./features-i18n.js";
...
export const DICT = {
  ...
  "inj.enforceSub": {
    en: "Checks files as they're written against the project's standards (toolchain versions and similar) and stops a write that breaks one; when a dependency manifest changed, also flags outdated library versions at the end of the turn. Turn it off for projects these checks don't fit — pulling the rules on request stays available.",
    ar: "يفحص الملفات وقت كتابتها مقابل معايير المشروع (نسخ أدوات اللغة وما شابه) ويوقف الكتابة التي تخالف واحدًا منها، وإذا تغيّر ملف المكتبات نبّه في آخر الرد إلى النسخ القديمة. أوقفه للمشاريع التي لا تناسبها هذه الفحوص — سحب القواعد بطلبك يبقى متاحًا.",
  },
  ...
  // ===== features tour page (assets/features-i18n.js) =====
  ...FEATURES_DICT,
};
